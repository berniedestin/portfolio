// portfolio-projects.jsx — live mini-animations for project cards.

// ── Setback (Trouble clone) — animated dice + token race ──────────
function SetbackAnim() {
  ...
  React.useEffect(() => {
    ...
    let dice = 1, dieFrame = 0, dieRoll = 0;
    let phase = 'roll'; // roll → move → next
    let phaseT = 0;
    let mover = 0;
...
function EtchAnim() {
  ...
  React.useEffect(() => {
    ...
    size();
    const ro = new ResizeObserver(() => requestAnimationFrame(() => { size(); cells.length = 0; init(); })); ro.observe(canvas);

    let cols = 0, rows = 0, cell = 0;
    let cells = []; // filled
    let head = { x: 0, y: 0, dx: 1, dy: 0 };
    let stepT = 0;
    let hue = 22; // around accent
    ...
    function step() {
      ...
      cells.push({ x: head.x, y: head.y, h: hue, t: 1 });
      hue = (hue + 0.5) % 360;
      if (cells.length > Math.floor(cols * rows * 0.55)) {
        cells.splice(0, 60);
      }
    }
    ...
    function draw(now) {
      ...
      cells.forEach(c => {
        const col = `hsl(${c.h}, 80%, ${dark?60:48}%)`;
        ctx.fillStyle = col;
...
function CanvasSandboxAnim() {
  ...
  React.useEffect(() => {
    ...
    function draw() {
      ...
      // attractor at center w/ rotation
...
// ── Drag/UI loop demo (placeholder project 4) ──────────────────
function DragDemoAnim() {
  const ref = React.useRef(null);
  React.useEffect(() => {
    const canvas = ref.current; if (!canvas) return;
    const ctx = canvas.getContext('2d');
    ...
    function draw() {
      ...
      // scripted demo: pick up item 2, drop it before item 0 every cycle
      const cyc = t % 6;
      if (cyc < 0.5) {
        ...
        if (dragging >= 0) {
          items[dragging].y += (0.5 - items[dragging].y) * 0.18;
        }
      } else if (cyc < 3.6) {
        // drop: reorder
        if (dragging >= 0) {
        ...
        if (cyc > 5.5) {
          // reset
          items.forEach((it, i) => { it.y = i; it.done = i < 2; it.hue = i < 2 ? 130 : 22; });
        }
      }
      ...
      ctx.beginPath();
      ctx.moveTo(cxp, cyp);
      ctx.lineTo(cxp+12, cyp+4);
      ctx.lineTo(cxp+5, cyp+5);
      ctx.lineTo(cxp+8, cyp+13);
      ctx.lineTo(cxp+5, cyp+14);
      ctx.lineTo(cxp+2, cyp+7);
    ...
    function draw() {
      ...
      function node(x, y, d, parent) {
        ...
        if (parent) {
          ...
          const tx = parent.x + (x-parent.x)*grown;
          const ty = parent.y + (y-parent.y)*grown;
          ctx.lineTo(tx, ty);
          ctx.stroke();
        }
        ...
        for (let i = 0; i < branch; i++) {
          const nx = tx + (i - (branch-1)/2) * spread;
          const ny = ty + 32;
          node(nx, ny, d+1, { x: tx, y: ty });
        }
      }
      node(cx, 28, 0, null);
...
Object.assign(window, { SetbackAnim, EtchAnim, CanvasSandboxAnim, DragDemoAnim, CodeOutputAnim });
