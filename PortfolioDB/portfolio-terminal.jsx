// portfolio-terminal.jsx
// Full-screen terminal mode with command parser + history.
// Triggered by status-bar button or backtick (`) key.
// Type "home" or "exit" to leave; "help" to see commands.
...
function PortfolioTerminal({ open, onClose, theme, setTheme, accent, setAccent, font, setFont, projects, scrollTo }) {
  ...
  // hard global escape — captures even when focus is anywhere
  React.useEffect(() => {
    if (!open) return;
  ...
  function neofetch() {
    ...
    const lines = [
      ...
      ['shell',  '/bin/destinsh'],
      ['kernel', 'react 18.3.1 / babel 7.29'],
      ['theme',  theme],
      ['accent', accent],
      ['font',   font],
      ['uptime', up],
    ...
    return [
      ...lines.map(([k,v]) => ({ kind: 'gridrow', cmd: k, desc: v })),
      { kind: 'spacer' },
    ];
  }
  ...
  function openProject(slug) {
    ...
    if (!p.url) return [{ kind: 'err', text: `open: ${slug}: no live URL.` }, { kind: 'spacer' }];
  ...
  function run(raw) {
    ...
    if (!trimmed) { emit(prompt('')); return; }
    const out = [prompt(trimmed)];
    ...
    switch (cmd.toLowerCase()) {
      case 'help': case '?':       out.push(...help()); break;
      ...
      case 'neofetch': case 'fetch': out.push(...neofetch()); break;
      case 'echo':                 out.push({ kind: 'out', text: arg }, { kind: 'spacer' }); break;
      case 'date':                 out.push({ kind: 'out', text: new Date().toString() }, { kind: 'spacer' }); break;
      case 'sudo':                 out.push({ kind: 'err', text: 'destin is not in the sudoers file. This incident will be reported.' }, { kind: 'spacer' }); break;
      case 'rm': case 'rm-rf':     out.push({ kind: 'err', text: "i'm not gonna let you do that." }, { kind: 'spacer' }); break;
      case 'vim': case 'emacs':    out.push({ kind: 'out', text: "we don't talk about editor wars here." }, { kind: 'spacer' }); break;
      case 'pwd':                  out.push({ kind: 'out', text: '/home/destin/portfolio' }, { kind: 'spacer' }); break;
      case 'banner':               out.push({ kind: 'banner' }); break;
      default:
        out.push({ kind: 'err', text: `command not found: ${cmd}` }, { kind: 'out', text: 'type ' + tag('help', 'key') + ' for available commands.' }, { kind: 'spacer' });
    }
    emit(...out);
  }
  ...
  // render rich tokens like <<key|text>>
  ...
  return (
    <div className={'term-mode' + (open ? ' on' : '')} aria-hidden={!open}
         onMouseDown={() => inputRef.current && inputRef.current.focus()}>
      <div className="term-scan" />
      <div className="term-head">
        <button type="button" className="lights" onClick={(e) => { e.stopPropagation(); onClose(); }}
                style={{padding:0, background:'transparent', border:0, cursor:'pointer'}}
                aria-label="Close terminal" title="close (esc)">
        ...
        <button type="button" onClick={(e) => { e.stopPropagation(); onClose(); }}
                style={{color:'#5b6270', background:'transparent', border:'1px solid #1a1d22',
